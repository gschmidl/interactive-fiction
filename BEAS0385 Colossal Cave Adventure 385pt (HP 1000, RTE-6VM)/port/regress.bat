...
REM files in tests\ were captured from ADVEN.RUN running under SIMH off the
...
if not exist "%~dp0build\adven.exe" (
  echo build\adven.exe not found -- run build.bat first.
  exit /b 1
)
...
for %%W in (walk1 walk2) do (
  del /q "#ADVXX" 2>nul
  adven.exe < "%~dp0tests\%%W.in" > "%~dp0tests\%%W.port.out" 2>&1
  echo|set /p="%%W: "
