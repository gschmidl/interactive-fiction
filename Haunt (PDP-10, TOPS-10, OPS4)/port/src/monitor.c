/* monitor.c -- the slice of the TOPS-10 / TYMCOM-X monitor that EXPLOR needs.
 *
...
const char *opt_corefile = "haunt.core";
 ...
 * Meta-commands.
 *
 * Haunt has no save command.  There was never one to find: the game
 * simply does not have that feature, so the only save it can be given
 * is a snapshot of the emulated machine, taken from outside it.
 *
 * A line beginning with # is answered here and never reaches the game,
 * so it costs no turn and the parser never sees it.  This is a safe
 * moment to snapshot: the input buffer is empty (that is why tty_fill
 * was called at all), and the instruction underway is an XCT of the
 * TTCALL in AC 11 -- INCHRW, a single-character read whose store into
 * memory happens only after tty_in returns.  Nothing has been written
 * yet.  Saving PC-1 therefore resumes by re-executing that read, which
 * simply waits for input again. */
static void meta_command(const char *line)
{
    const char *a = line + 1;
    char verb[16];
    int i = 0;
    while (*a == 32) a++;
    while (i < 15 && *a && *a != 32) verb[i++] = (char)tolower((unsigned char)*a++);
    verb[i] = 0;
    while (*a == 32) a++;
    if (!strcmp(verb, "save")) {
        const char *path = *a ? a : opt_corefile;
        int here = PC;
        PC = (PC - 1) & HMASK;
        if (save_core(path))
            printf("[saved in %s -- \"haunt -c\" resumes it]\n", path);
        else
            printf("[could not write %s]\n", path);
        PC = here;
    } else if (!strcmp(verb, "help") || !strcmp(verb, "?")) {
        printf("[#save [file] writes a snapshot; the game itself has no save.]\n");
    } else {
        printf("[no such meta-command: %s -- try #help]\n", verb);
    }
    fflush(stdout);
}
