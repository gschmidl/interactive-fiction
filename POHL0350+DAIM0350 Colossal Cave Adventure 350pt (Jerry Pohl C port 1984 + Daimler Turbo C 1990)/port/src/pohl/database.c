...
/*
	...
	end in its data segment - idx1 and idx2, then idx3 with MAXOBJ slots
	(the last 36 zero), then idx4 - so an index below 0 read the tables
	before: rspeak(0) read idx3[MAXOBJ-1], 0, and printed advent4.txt from
	its first byte.  Here the same values are looked up explicitly.
*/
