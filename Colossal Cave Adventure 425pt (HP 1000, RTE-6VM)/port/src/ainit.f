...
C 
C  GET RUNSTRING AND SAVE. START THE TIMER RUNNING. 
C 
      CALL FPARM(FILENAME)
      CALL TIMER(0) 
C 
...
C 
C  SET RANDOM NUMBER SEED BY USING DATE AND TIME
C 
      CALL FTIME(ITEXT) 
      LEN=0 
      DO I=1,15 
      ...
      CALL SSEED(LEN) 
C 
C  GET FILE NAME FROM RUNSTRING TO RESTORE, OTHERWISE USE DEFAULT.
C 
      ...
      LEN=NFIOB(0)
D     WRITE(CRT,40) LEN 
D  40 FORMAT("Number of disc buffers=",I3)
     ...
     &     ACCESS='SEQUENTIAL',FORM='UNFORMATTED',ERR=200) 
      ...
      IF(REV.EQ.REVISION) THEN  
        CALL RESTORE(FILENAME)
        IF(FILENAME.EQ.'ADVENTURE.DAT') RESTART=.FALSE. 
        GO TO 300 
      ELSE
        WRITE(CRT,100) REV,FILENAME(1:FILELEN)
  100   FORMAT(/"Cannot use revision",F4.1, 
     &         ", of file ",A,", aborting."/) 
        CALL EXIT 
      ENDIF 
C 
...
C 
  ...
  210   FORMAT(/"Cannot find ",A,", run abuild.exe to create data base."/)
      ELSE
  ...
  220   FORMAT(/"FORTRAN File Error",I6," on file ",A)
      ...
      CALL EXIT 
C 
C  CHECK FOR PRIME TIME, DEMO OR WIZARD 
C 
  300 WIZARD=.FALSE.
      IF(PRIMTIM(0)) THEN 
        CALL MSPEK(2) 
        CALL HOURS
        IF(YESX(4,5,5,MSPEK)) THEN
          DEMO=.TRUE. 
          RETURN
        ENDIF 
        CALL MSPEK(3) 
        IF(.NOT.GUARD(0)) CALL EXIT 
      ENDIF 
C 
D     WRITE(CRT,'("INIT Done")')
      RETURN
      END 
