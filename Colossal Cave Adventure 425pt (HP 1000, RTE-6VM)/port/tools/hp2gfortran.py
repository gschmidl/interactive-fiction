...
CTRLLINE = re.compile(r'^\s*FTN(4|77|7X)(?![A-Z0-9])', re.I)
UNITHEAD = re.compile(
...
def convert(lines, incmap):
    ...
    for raw in lines:
        ...
        if m:
            ...
            continue
        if DIRECTIVE.match(line) or CTRLLINE.match(line):
            res.append('C     ' + line.strip())
            continue
        ...
        if not in_call:
            line = holleriths(line)
        line = widths(line)
        res.append(OCTAL.sub(lambda mm: str(int(mm.group(1), 8)), line))
    return res
