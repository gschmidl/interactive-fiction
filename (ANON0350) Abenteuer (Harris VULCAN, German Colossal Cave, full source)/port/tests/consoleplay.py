"""The game at a real console -- the way play.bat is played.

    python tests/consoleplay.py [abenteuer.exe]

The "window" is a Windows pseudo console (ConPTY): the program sees a real
console and nothing appears on the desktop.  It runs a scratch copy of the
port (saves\\ and NEUSPIEL.DAT there), with the clock held on a Sunday
evening so that the site's hours do not close the cave.  Checked on a
model of the screen:

  1. The game goes on from NEUSPIEL: "DU BIST AM ENDE DER STRASSE.", and
     the program waits on the line below a blank one (GETIN writes the
     blank line, then reads), the cursor at its start.
  2. A command typed there shows once: the console echoes it (line
     input), the program does not; lower case is taken.
  3. AUFGABE, JA: the program ends by itself, exit code 0.
"""
import ctypes
...
EXE = os.path.join(ROOT, 'abenteuer.exe')
CWD = ROOT
...
def waiting(s):
    """GETIN's blank line written, the cursor at the start of the next"""
    return s.c == 0 and s.row(s.r) == '' and s.r > 0 and s.row(s.r - 1) == ''


def main():
    global CWD
    exe = sys.argv[1] if len(sys.argv) > 1 else EXE
    CWD = tempfile.mkdtemp(prefix='abenteuercon')
    for f in ('ADV.DATA', 'NEUSPIEL.DAT'):
        shutil.copy(os.path.join(os.path.dirname(exe), f), CWD)
    shutil.copy(exe, os.path.join(CWD, 'abenteuer.exe'))
    w = Window('abenteuer', [os.path.join(CWD, 'abenteuer.exe'),
                             '--date', '2026-09-20', '--time', '20:00'])
    try:
        w.wait_for('DU BIST AM ENDE DER STRASSE.')
        w.wait_screen(waiting)
        w.type('osten\r')
        w.wait_for('HIER IST EINE FLASCHE WASSER.')
        w.wait_screen(waiting)
        rows = [w.screen.row(k) for k in range(w.screen.rows)]
        if rows.count('osten') != 1:
            raise AssertionError('"osten" is not shown once on its own line:\n'
                                 + w.screen.text())
        if 'DU BEFINDEST DICH INNERHALB EINES GEBAEUDES.' not in rows:
            raise AssertionError('no building:\n' + w.screen.text())
        w.type('aufgabe\r')
        w.wait_for('WILLST DU WIRKLICH AUFGEBEN?')
        w.type('ja\r')
        w.wait_for('DU ERZIELTEST')
        code = w.wait_exit(30)
        if code != 0:
