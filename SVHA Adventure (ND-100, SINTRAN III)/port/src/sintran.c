...
static int mon_open(Cpu *c)
{
    ...
    if (access != 1 && access != 3 && access != 7)
        return fail(c, E_NOT_WRITE_ACCESS);   /* the port never writes the game's files */
...
static int mon_rfile(Cpu *c)
{
    ...
    for (i = 0; i < words; i++) {
        int hi = fgetc(sf->f), lo = fgetc(sf->f);
        if (hi == EOF || lo == EOF) {
            R(A) = E_EOF;
            return SIN_RUN;
        }
        M(buf + i) = (uint16_t)((hi << 8) | lo);
    }
...
int sintran_mon(Cpu *c, int n)
{
    ...
    case 2: {                                   /* OUTBT: T = device, A = byte */
        int no = (int16_t)R(T);
        if (!is_terminal(no)) {
            logcall(c, n, "OUTBT (not the terminal)");
            return fail(c, file_of(no) ? E_NOT_WRITE_ACCESS : E_NOT_OPEN);
        }
        term_putc(R(A));
        if ((R(A) & 0x7F) == '\n')
            snt.out_tail_len = 0;
        else if (snt.out_tail_len < (int)sizeof snt.out_tail)
            snt.out_tail[snt.out_tail_len++] = (char)(R(A) & 0x7F);
        skip(c);
        return SIN_RUN;
    }
    ...
    case 073:                                   /* SMAX: T = file, AD = bytes - 1 */
        logcall(c, n, "SMAX");
        if (!file_of((int16_t)R(T)))
            return fail(c, E_NOT_OPEN);
