"""A game at a real console -- the way play.bat is played.
...
desktop.  What pipes cannot show is checked here, on a model of the screen:
  ...
  1. The prompt " >" is a record ending in '_' (REIO): no new line after
     it, so a command is typed on the line of its prompt.
  2. So is SAVE's "ENTER FILE NAME: " (a FORMAT ending in _).
  3. QUIT, Y: the program ends by itself (EXEC 6), with exit code 0.

It runs in a scratch directory with a copy of the data base (never the
real saves\\).
"""
import ctypes
...
EXE = os.path.join(ROOT, 'dungeon.exe')
CWD = ROOT
...
def main():
    ...
    CWD = tempfile.mkdtemp(prefix='dngcon')
    shutil.copy(exe, os.path.join(CWD, 'dungeon.exe'))
    for f in ('@DUNGN', '@DUNGT', '@DUNGI'):
        shutil.copy(os.path.join(os.path.dirname(exe), f),
                    os.path.join(CWD, f))
    w = Window('dungeon', [os.path.join(CWD, 'dungeon.exe'),
                           '--time', '10:00', '--date', '1982-11-18'])
    try:
        w.wait_for('There is a small mailbox here.')
        w.wait_screen(lambda s: s.row(s.r) == ' >' and s.c == 2)
        w.type('open mailbox\r')
        w.wait_for('A leaflet.')
        w.wait_screen(lambda s: s.row(s.r) == ' >' and s.c == 2)
        rows = [w.screen.row(k) for k in range(w.screen.rows)]
        if ' >OPEN MAILBOX' not in rows and ' >open mailbox' not in rows:
            raise AssertionError('the command is not on the line of the '
                                 'prompt:\n' + w.screen.text())
        w.type('save\r')
        w.wait_for('ENTER FILE NAME:')
        w.wait_screen(lambda s: s.row(s.r).endswith('ENTER FILE NAME:'))
        w.type('G1\r')
        w.wait_for('Saved.')
        w.wait_screen(lambda s: s.row(s.r) == ' >' and s.c == 2)
        rows = [w.screen.row(k) for k in range(w.screen.rows)]
        if not any(r.startswith('ENTER FILE NAME: G1') for r in rows):
            raise AssertionError('the file name is not on the line of its '
                                 'question:\n' + w.screen.text())
        if any(r.endswith('_') for r in rows):
            raise AssertionError('a record ends in _:\n' + w.screen.text())
        if not os.path.exists(os.path.join(CWD, 'saves', 'G1')):
            raise AssertionError('no saves\\G1')
        w.type('quit\r')
        w.wait_for('Do you wish to leave the game?')
        w.type('y\r')
        code = w.wait_exit(30)
        if code != 0:
