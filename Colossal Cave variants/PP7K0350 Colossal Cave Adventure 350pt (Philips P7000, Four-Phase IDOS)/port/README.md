...
`build.sh` compiles `advent.exe` and copies the pack to `p7000.pack` next to it. `run.bat` runs the game with `-u`.
...
## Tests
    ...
    python tests/fuzz.py [GAMES] [TURNS] [SEED] [--real-clock]   random commands; three runs per game on
                                       one pack (--real-clock: on the 60 Hz clock as run.bat runs it;
    ...
    python tests/consoleplay.py        at a real console (ConPTY): SUSPEND and resume, QUIT, typing shown under
                                       the prompt, Backspace, Ctrl+C, run.bat
