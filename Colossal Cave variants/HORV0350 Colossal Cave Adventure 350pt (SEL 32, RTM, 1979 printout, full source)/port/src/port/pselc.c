/* PORT: the three site-supplied routines of Palter's conversion guide, and
   the file transfer that stands in for RTM's BUFFERIN/BUFFEROUT.
   ...
   "allows four contiguous INTEGER*4 variables for each pointer", which is
   just as well: a pointer here is eight bytes, and it goes in the first two
   of them.

   The SEL 32 was a 32-bit big-endian machine, so a word is four bytes there
   and here; the saved file is therefore the same words in the same order,
   only little-endian.  */
