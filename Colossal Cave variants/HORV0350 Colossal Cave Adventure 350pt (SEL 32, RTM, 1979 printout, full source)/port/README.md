...
`advent.exe` plays the 350-point game. Run `run.bat`. The program sets itself up on a first run - see
...
## Options
    ...
    advent [-u] [--auto] [--day N] [--time HHMM] [--no-fixes] [-h]

**The cave keeps the site's hours**: as `build.sh` sets the game up, it is shut 8:00 to 18:00 on
weekdays, when only a wizard (or the 30-turn demonstration game) gets in, and a suspended game may
not be restored for 90 minutes. `-u` (`--unlimited`) lifts both: `START` then finds no prime time,
so there is no demonstration game either, and skips the wait. `run.bat` passes it. The change is in
`convert.py` (`START`) and `popts.f`; without `-u` the game is as transcribed.
