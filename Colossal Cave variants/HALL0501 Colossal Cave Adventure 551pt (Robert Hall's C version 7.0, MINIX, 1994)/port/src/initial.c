...
void initialize()
{
    ...
    /* PORT: both loops began at MAXOBJ (or ran to it), one past plac[],
       fixd[] and g's arrays; the objects are 1 .. MAXOBJ-1, and every
       other loop in the program stops before MAXOBJ */
    for (i = MAXOBJ - 1; i > 0; i--) {
	g.fixed[i] = fixd[i];
	if (fixd[i] > 0) {
