...
turn()
{
	...
	*/
	/* PORT: Fix 1 - "cond[loc]&NOPIRAT == 0" is cond[loc] & (NOPIRAT == 0),
	   always 0, so no dwarf ever blocked the way; --no-fixes keeps that */
	if (newloc != loc && !forced(loc) && port_fixes && (cond[loc]&NOPIRAT) == 0)
...
/*	causes occasional "move" with no describe & descitem	*/
	...
	if (closed) {
		if (prop[OYSTER] < 0 && toting (OYSTER))
			pspeak(OYSTER, 1);
		for (i = 1; i < MAXOBJ; ++i)	/* PORT: was <= MAXOBJ, one past place[] and prop[] */
...
goback()
{
	...
	for (kk = 0; travel[kk].tdest != -1; ++kk) {
		...
		if (!travel[kk].tcond) {
			...
			/* PORT: Fix 5 - gettrav() only of a location (Woods:
			   "IF(LL.GT.300) GOTO 22"): of a special move or a
			   message it read cave[] far past its end */
			if (port_fixes && temp > MAXLOC)
				continue;
			gettrav(temp);
			/* PORT: Fix 5 - k2 = temp put a location number where
			   travel[] is indexed below, far past its end; with the
			   fixes the entry's own index, as in Pohl's.  --no-fixes
			   keeps Daimler's lines: what they read there is what
			   his DOS program did in tests\crossdos.py */
			if (forced(temp) && travel[0].tdest == want)
				k2 = port_fixes ? kk : temp;
...
death()
{
	...
	if (!closing) {
		...
		for (j = 1; j<101; ++j) {
			i = 101-j;
			if (i < MAXOBJ && toting (i))	/* PORT: i = 100 read one past place[] */
		...
		*/
		for (try = 1; try<20; ++try) {
			...
			if (j != odloc[i] && j != dloc[i] &&
			    /* PORT: Fix 2 - "cond[j]&NOPIRAT == 1" is cond[j] & 0,
			       so the pirate went anywhere; Pohl's own test */
			    !(i == (DWARFMAX-1) && port_fixes && (cond[j]&NOPIRAT)))
				break;
		}
		if (j == 0)
			j = odloc[i];
...
stimer()
{
	...
	if (clock2 == 0) {
		...
		*/
		...
		fixed[MIRROR] = 116;
		for (i = 1; i< MAXOBJ; ++i)	/* PORT: was <= MAXOBJ */
			if (toting(i))
				dstroy(i);
		rspeak(132);
		closed = 1;
