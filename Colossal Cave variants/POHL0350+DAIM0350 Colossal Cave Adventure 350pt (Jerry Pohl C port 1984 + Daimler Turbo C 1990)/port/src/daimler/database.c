...
int	msg1, msg2, msg3;
{
	...
	if (msg1)
		rspeak(msg1);
	fputc('>', stdout);
	if (!fgets(answer, 80, stdin)) {	/* PORT: end of input ends the game */
		fputc('\n', stdout);
		exit(0);
	}
...
/*
	PORT: idx4[i] as the DOS program read it.  The four tables lay end to
	end in its data segment (ADVENT.EXE: idx1 at 0x92a2, idx2 0x94d2, idx3
	0x9702 with MAXOBJ slots, the last 36 zero, idx4 0x9892), so an index
	below 0 read the tables before - which a message number above 127 did,
	kept in a char (see PORT_CHAR in portcompat.h), and rspeak(0) too.
	Here the same values are looked up explicitly.  Past the end (the -d
	trace below asks for idx4[201]) there is nothing to show: 0.
*/
static long port_idx4(i)
int	i;
{
	if (i >= MAXMSG)
		return 0L;
...
int	msg;
{
	if (msg == 54)
		...
		fseek(fd4, port_idx4(msg - 1), 0);	/* PORT: was idx4[msg - 1] */
		rdupto(fd4, '#', 1, 0);
	}
	return;
}
...
int	obj, where;
{
	...
	/* PORT: was fixed[obj], past the end of fixed[] for the second place
	   of a two-place object; carry() does nothing with such an obj, so
	   what was read there never mattered */
