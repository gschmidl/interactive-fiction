...
vtake()
{
	...
	*/
	...
	if (object == CHAIN && prop[BEAR] != 0)
		msg = 170;
	if (fixed[object]) {
		rspeak(PORT_CHAR(msg));	/* PORT: Fix 3 */
		return;
	}
	/*
...
int verb;
{
	...
	/* PORT: Fix 4 - actmsg[] lost LOG's entry (Pohl's 13) when Daimler
	   cut it to 32, so LOG with an object ended the game with "Fatal error
	   number 39"; with the fixes it gets Pohl's answer */
	if (port_fixes && verb == LOG) {
		rspeak(13);
		return;
	}
	if (verb<1 || verb>31)
