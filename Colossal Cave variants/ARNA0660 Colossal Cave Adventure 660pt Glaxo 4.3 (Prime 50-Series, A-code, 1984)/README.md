...
**Status: PORTED 2026-09-21 (first pass).** `port\adventure4.exe` (`port\run.bat`) runs the tape's database under the
...
directory `PULSE>ADVENTURE4`. Files cut out with `..\..\_work\_bits_sweep_work\tools\primex.py` (raw + decoded `.txt`).
