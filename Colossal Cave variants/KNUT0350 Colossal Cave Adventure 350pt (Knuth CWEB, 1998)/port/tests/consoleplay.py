...
A Windows pseudo console (ConPTY) runs advent.exe: the program sees a real
console and reads real key events, and nothing appears on the desktop.
What a pipe cannot show:

  1. the question and the "** " prompt are on the screen before the
     program waits for the answer, and the room and the "* " prompt before
     it waits for a command (output is flushed before every read);
  2. typed lower case is played;
  3. a line longer than the 72-byte buffer, typed at the console: the
     command at its start is played and the rest is dropped (Fix 3);
  4. QUIT and y: the score, and the program ends, exit 0;
  5. Ctrl-Z Enter at a command, and at the first question: the program
     ends at once, exit 0.
"""
import ctypes
...
class Window:
    ...
    def waiting_after(self, what, prompt):
        """WHAT and then PROMPT drawn, and nothing more: the program has
        shown the prompt and waits for the keyboard"""
        self.wait_for(what)
        self.wait_for(prompt)
        time.sleep(0.3)
        rest = self.drawn()[self.mark:]
        if rest or self.exited():
            raise AssertionError('not waiting after %r: %r' % (prompt, rest[:200]))
...
def main():
    tmp = tempfile.mkdtemp(prefix='advent-console-')
    exe = os.path.join(tmp, 'advent.exe')
    shutil.copy(os.path.join(PORT, 'advent.exe'), exe)
    failed = []
    ...
    def game():
        win = Window([exe], tmp)
        try:
            if not step('the question and "** " before the first read',
                        lambda: win.waiting_after('Would you like instructions?', '**')):
                return

            def lower():
                win.type('no\r')
                win.waiting_after(ROAD, '*')
                win.type('in\r')
                win.waiting_after(HOUSE, '*')
            if not step('typed lower case; the room and "* " before each read', lower):
                return

            def long_line():
                win.type('look' + ' ' * 70 + 'xyzzy\r')
                win.waiting_after('I will repeat the long description', '*')
            if not step('a line past the buffer: its start is played', long_line):
                return

            def quit_():
                win.type('quit\r')
                win.wait_for('Do you really want to quit now?')
                if 'Nothinghappens.' in win.drawn():
                    raise AssertionError('the rest of the long line was a command')
                win.type('y\r')
                win.wait_for('You scored')
                ends_ok(win)
            step('... not its rest; QUIT, y: the score, exit 0', quit_)
        finally:
            win.close()

    def ctrl_z(name, before):
        win = Window([exe], tmp)
        try:
            def fn():
                win.waiting_after('Would you like instructions?', '**')
                if before:
                    win.type(before)
                    win.waiting_after(ROAD, '*')
                win.type('\x1a\r')
    ...
    try:
        game()
        ctrl_z('Ctrl-Z Enter at a command: the end, exit 0', 'no\r')
        ctrl_z('Ctrl-Z Enter at the first question: the same', '')
    finally:
