...
EDITS = [
    ...
    ('      MASKC = -MASKC',
     ['C     MASKC = -MASKC - unary minus complements a word on a CDC 6000',
      '      MASKC = PNOT(MASKC)'], 1),
    # ---- -u (--unlimited, pjaze.f's POPTS): never prime time, and no wait
    #      before a suspended game may be resumed
    ('      PTIME=IAND(PRIMTM,SHIFT(1,T/60)).NE.0',
     ['      PTIME=IAND(PRIMTM,SHIFT(1,T/60)).NE.0',
      '      IF(PUNL.NE.0)PTIME=.FALSE.'], 1),
    ('      IF(SETUP.GE.0)GOTO 20',
     ['      IF(SETUP.GE.0.OR.PUNL.NE.0)GOTO 20'], 1),
]
