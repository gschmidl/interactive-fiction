/* main.c -- driver for the CRYSTAL CAVE port.
 *
 * Loads the repaired CAVE core image and runs it on the emulated
 ...
 *                 expedition of the 30 turns at 056540, or nothing
 ...
 *                 hours.
 *   056543        latency for restart, in minutes -- 90.  The same
 ...
 *   004462        JUMPL 2,13761 -- the short expedition has used its
 ...
 * SUSPEND writes cave.core, so a game saved under -u is an ordinary
...
{
    printf(
    "CAVE -- CRYSTAL CAVE, a Tymshare DECsystem-10 adventure, run as the\n"
    "original FORTRAN binary on a PDP-10 emulator.\n"
    "\n"
    "usage: cave [options]\n"
...
int main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (!strcmp(a, "-c") || !strcmp(a, "--continue")) resume = 1;
            ...
            fprintf(stderr, "cave: unknown option %s (try -h)\n", a);
            return 1;
        }
    }
    ...
    if (resume && !load_core(corefile)) {
        fprintf(stderr, "cave: no saved game in %s\n", corefile);
        return 1;
    }

    if (opt_unlimited && !unlimited_on()) {
        fprintf(stderr, "cave: -u found nothing to patch in this image\n");
        return 1;
    }
     ...
     * it did on TYMCOM-X.  Do it for the player; "cave -c" picks the
    ...
    if (suspend_requested) {
        ...
        if (save_core(corefile))
            printf("\n[core image saved in %s -- \"cave -c%s\" to continue]\n",
