...
## Options
    ...
    advent [-u] [--seed N] [--time HHMM] [--no-fixes] [-h]

**The cave is shut** from 06.00 to 11.30 and from 13.30 to 15.30 - the E.P.A.
has stated that Middle Earth may not be entered during those hours. TIMER
stops the game at the start, and MAIN ends it ("YOU LOOK AT YOUR WATCH...") when
a turn falls inside them, unless you answered `YES` to "perchance, are you a
wizard?" and gave the password, `WORMTONGUE`; then the clock is off.

`-u` (`--unlimited`) turns the clock off as the password does, so the hours
never apply; the wizard's question is still asked, and NO is fine. `run.bat`
passes it. Without `-u` the game is the site's: `convert.py` makes `TIMER` set
`WIZSW` from `PUNLIM()` instead of `.FALSE.`, and that is the only change.
