...
C  -u: the cave's hours do not apply.  TIMER then turns the clock off,
C  as it did for the wizard.
      LOGICAL FUNCTION PUNLIM()
      IMPLICIT INTEGER (A-Z)
      COMMON /PUNCOM/ PUNL
      PUNLIM=PUNL.NE.0
      END
...
2        FORMAT(' PORT: unknown option ',A)
      ...
      PRINT 1
1     FORMAT(
     +' adventure [-u] [--seed N] [--time HHMM] [--no-fixes] [-h]'//
     ...
     +' FORTRAN IV.'//
     +'   -u, --unlimited  the cave''s hours do not apply (the clock'/
     +'                is off, as it was for the wizard); run.bat'/
     +'                passes it'/
