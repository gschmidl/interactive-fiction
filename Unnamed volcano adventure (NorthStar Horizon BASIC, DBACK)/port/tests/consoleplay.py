"""The game at a real console -- the way play.bat is played.
    ...
    python tests/consoleplay.py [volcano.exe]
...
desktop.  What pipes cannot show is checked here, on a model of the screen:
  ...
  1. "MOVE? " waits with the cursor after it, and a key typed there shows
     once: HYBASIC echoes it, the console does not (its echo is off).
  2. Backspace rubs a character out on the screen too, and HYBASIC drops
     it from the move.
  3. The game's end (the pit) ends the program, exit code 0, without
     HYBASIC's READY.
"""
import ctypes
...
EXE = os.path.join(ROOT, 'volcano.exe')
CWD = ROOT
...
def prompt(s):
    return s.row(s.r) == 'MOVE?' and s.c == 6


def main():
    exe = sys.argv[1] if len(sys.argv) > 1 else EXE
    w = Window('volcano', [exe, '--seed', '0'])
    try:
        w.wait_for('YOU A CHANCE TO ESCAPE')
        w.wait_screen(prompt)
        w.type('x')
        w.wait_screen(lambda s: s.row(s.r) == 'MOVE? X' and s.c == 7)
        w.type('\x08')
        w.wait_screen(prompt)
        w.type('u\r')
        w.wait_for('POISED ON THE EDGE')
        w.wait_screen(prompt)
        rows = [w.screen.row(k) for k in range(w.screen.rows)]
        if 'MOVE? U' not in rows:
            raise AssertionError('the move is not "MOVE? U":\n'
                                 + w.screen.text())
        w.type('d\r')
        w.wait_for('YOU PLUNGE OVER THE EDGE')
        w.wait_screen(prompt)
        w.type('pit\r')
        w.wait_for('PLEASE COME BACK NEXT WEEK')
        code = w.wait_exit(30)
        if code != 0:
            raise AssertionError('exit code %d' % code)
        time.sleep(0.3)
        if 'READY' in w.screen.text():
            raise AssertionError('READY shown:\n' + w.screen.text())
        print('console ok')
        return 0
