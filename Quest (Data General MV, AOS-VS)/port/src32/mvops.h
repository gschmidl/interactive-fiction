...
static const mvop *mvfind(unsigned short ir)
{
    ...
    /* XVCT and QSCAN in their CF.. form are ONE word.  These are the traps
     * the F77 compiler plants after a range check -- "skip me if the
     * subscript is in bounds" -- and the compiled code is unambiguous about
     * their length.  QUEST's initials check reads
     *
     *   C9A9  WSLE 2,1     skip if 2 <= len
     *   CF09  XVCT         trap
     *   8D89  WSBI 1,1
     *   C629 0001  NLDAI 0,1
     *   89A9  WSLE 0,1
     *   CF09  XVCT
     *   8BC9 7000 021D  LNLDA 1,IN_BUFFER+1   <- the initials
     *   8BD9 7000 0CCC  LNSTA 1,...
     *
     * and QUEST_SERVER's world set-up reads CF19 / CE29 0009 / A169, an
     * NLDAI and a WMUL.  Read as two words each, the skip lands one word
     * late and the machine executes an operand: QUEST fell into its own
     * IN_BUFFER address and QUEST_SERVER jumped to location 9.
     *
     * The C7.. forms keep the length MASM's table gives them.  ZORK.PR's
     * storage allocator walks its free list with 0xC719 followed by an
     * inline word and is decoded that way in wideexec.h; only the CF..
     * forms appear in compiled F77, 75 XVCT in QUEST.PR, 5 in
     * QUEST_SERVER.PR, 67 in DISCO.PR, and no C7.. form occurs as an
     * instruction anywhere -- the single 0xC709 in MASM.PR is its own
     * table entry.  Whether bit 11 really selects the length or the two
     * uses are different instructions that share an encoding is not
     * settled here; what is settled is that these two forms are one word. */
    {
