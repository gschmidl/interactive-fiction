"""Play random games of DOD on the port and look for trouble.
...
kept (DIR\portNN.keys).  Each seed has dice of its own (--uptime
...
PROG = os.path.join(DATA, 'DOD-ENB.PROG')
...
def play(seed, commands, work, keep=None, first=None, fixed=False):
    ...
    try:
        ...
        while problem is None:
            ...
            if choice is None and player.done >= player.commands:
                break                       # enough: the end of the input ends it
            if choice is None:
                time.sleep(1.0)
                if r.p.poll() is not None:
                    break                   # it had ended, only slowly
                choice = player.answer(r.tail())
                if choice is None:
...
def main():
    ...
    for seed in range(1, seeds + 1):
        work = tempfile.mkdtemp(prefix='dodfuzz')
        try:
