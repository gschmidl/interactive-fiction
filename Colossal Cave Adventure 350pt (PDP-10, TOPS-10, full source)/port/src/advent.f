...
C	100 OBJECTS
	...
	COMMON /ADVSTA/ DSEEN,HINTED,TRAVEL,LTEXT,STEXT,KEY,COND,
	1	PLAC,FIXD,PROP,ACTSPK,CTEXT,CVAL,HINTLC,HINTS,TK,DLOC,
	1	ODLOC,IDONDX,WZDARK,LMWARN,CLOSNG,PANIC,CLOSED,GAVEUP,
	1	SCORNG,DEMO,YEA,LINSIZ,TRVSIZ,LOCSIZ,VRBSIZ,RTXSIZ,CLSMAX,
	1	HNTSIZ,MAGSIZ,I,LINUSE,TRVS,CLSSES,SECT,OLDLOC,LOC,J,KK,K,
	1	NEWLOC,L,TABNDX,OBJ,VERB,HNTMAX,MAXTRS,TALLY,TALLY2,KEYS,
	1	LAMP,GRATE,CAGE,ROD,ROD2,STEPS,BIRD,DOOR,PILLOW,SNAKE,
	1	FISSUR,TABLET,CLAM,OYSTER,MAGZIN,DWARF,KNIFE,FOOD,BOTTLE,
	1	WATER,OIL,PLANT,PLANT2,AXE,MIRROR,DRAGON,CHASM,TROLL,
	1	TROLL2,BEAR,MESSAG,VEND,BATTER,NUGGET,COINS,CHEST,EGGS,
	1	TRIDNT,VASE,EMRALD,PYRAM,PEARL,RUG,CHAIN,BACK,LOOK,CAVE,
	1	NULL,ENTRNC,DPRSSN,SAY,LOCK,THROW,FIND,INVENT,CHLOC,
	1	CHLOC2,DFLAG,DALTLC,TURNS,IWEST,KNFLOC,DETAIL,ABBNUM,
	1	MAXDIE,NUMDIE,DKILL,FOOBAR,BONUS,CLOCK1,CLOCK2,LIMIT,
	1	DTOTAL,ATTACK,STICK,OLDLC2,SPK,HINT,WD1,WD1X,WD2,WD2X,KQ,
	1	LL,SPICES,K2,SCORE,MXSCOR,FOO
...
1002	CALL RDFRE(1,DBBUF,1)
	SECT=DBBUF(1)
C1003	FORMAT(G)		-- DEC free format, see RDFRE
	OLDLOC=-1
...
C1005	FORMAT(1G,15A5)		-- packed text, see RDMSG
	IF(KK.NE.17315143744 )CALL BUG(0)
	IF(LOC.EQ.-1)GOTO 1002
	DO 1006 K=1,14
	KK=LINUSE+15-K
	IF(LINES(KK).NE.17315143744 )GOTO 1007
1006	CONTINUE
...
1042	ATAB(TABNDX)=ATAB(TABNDX).XOR.-25465106546 
	CALL BUG(4)
...
C  DEFINE SOME HANDY MNEMONICS.  THESE CORRESPOND TO OBJECT NUMBERS.
	...
	KEYS=VOCAB(0+-28161813696 ,1)
	LAMP=VOCAB(0+-27642113984 ,1)
	GRATE=VOCAB(0+-30255557494 ,1)
	CAGE=VOCAB(0+-32474151616 ,1)
	ROD=VOCAB(0+-24362475456 ,1)
	ROD2=ROD+1
	STEPS=VOCAB(0+-23804587866 ,1)
	BIRD=VOCAB(0+-32977107904 ,1)
	DOOR=VOCAB(0+-31878294976 ,1)
	PILLOW=VOCAB(0+-25461109602 ,1)
	SNAKE=VOCAB(0+-23829886070 ,1)
	FISSUR=VOCAB(0+-30829587542 ,1)
	TABLET=VOCAB(0+-23347508086 ,1)
	CLAM=VOCAB(0+-32428208832 ,1)
	OYSTER=VOCAB(0+-25930640246 ,1)
	MAGZIN=VOCAB(0+-27105443404 ,1)
	DWARF=VOCAB(0+-31845199220 ,1)
	KNIFE=VOCAB(0+-28124592502 ,1)
	FOOD=VOCAB(0+-30804556736 ,1)
	BOTTLE=VOCAB(0+-32951872360 ,1)
	WATER=VOCAB(0+-21736307292 ,1)
	OIL=VOCAB(0+-25997991872 ,1)
	PLANT=VOCAB(0+-25448886616 ,1)
	PLANT2=PLANT+1
	AXE=VOCAB(0+-33451499456 ,1)
	MIRROR=VOCAB(0+-27071524194 ,1)
	DRAGON=VOCAB(0+-31866173538 ,1)
	CHASM=VOCAB(0+-32444984422 ,1)
	TROLL=VOCAB(0+-23275778920 ,1)
	TROLL2=TROLL+1
	BEAR=VOCAB(0+-32994438592 ,1)
	MESSAG=VOCAB(0+-27088268414 ,1)
	VEND=VOCAB(0+-22256597870 ,1)
	BATTER=VOCAB(0+-33010592630 ,1)

C  OBJECTS FROM 50 THROUGH WHATEVER ARE TREASURES.  HERE ARE A FEW.

	NUGGET=VOCAB(0+-30267784128 ,1)
	COINS=VOCAB(0+-32415363418 ,1)
	CHEST=VOCAB(0+-32444853336 ,1)
	EGGS=VOCAB(0+-31375240384 ,1)
	TRIDNT=VOCAB(0+-23275977590 ,1)
	VASE=VOCAB(0+-22273211072 ,1)
	EMRALD=VOCAB(0+-31350140286 ,1)
	PYRAM=VOCAB(0+-25393806950 ,1)
	PEARL=VOCAB(0+-25478245736 ,1)
	RUG=VOCAB(0+-24337211328 ,1)
	CHAIN=VOCAB(0+-32444986980 ,1)

C  THESE ARE MOTION-VERB NUMBERS.

	BACK=VOCAB(0+-33011152064 ,0)
	LOOK=VOCAB(0+-27583329472 ,0)
	CAVE=VOCAB(0+-32473660096 ,0)
	NULL=VOCAB(0+-26484519872 ,0)
	ENTRNC=VOCAB(0+-31345454462 ,0)
	DPRSSN=VOCAB(0+-31920205174 ,0)

C  AND SOME ACTION VERBS.

	SAY=VOCAB(0+-23883636672 ,2)
	LOCK=VOCAB(0+-27583722688 ,2)
	THROW=VOCAB(0+-23317622866 ,2)
	FIND=VOCAB(0+-30829755328 ,2)
	INVENT=VOCAB(0+-29197908580 ,2)
...
6000	IF(DFLAG.NE.1)GOTO 6010
	LPCTT=0
	IF(PCT(95))LPCTT=1
	IF(LOC.LT.15.OR.LPCTT.NE.0)GOTO 2000
	DFLAG=2
	DO 6001 I=1,2
	J=1+RAN10(5)
C  IF SAVED NOT = -1, HE BYPASSED THE "START" CALL.
	LPCTT=0
	IF(PCT(50))LPCTT=1
6001	IF(LPCTT.NE.0.AND.SAVED.EQ.-1)DLOC(J)=0
	DO 6002 I=1,5
...
6020	IF(HERE(IDONDX))K=1
	...
	LPCTT=0
	IF(PCT(20))LPCTT=1
	IF(ODLOC(6).NE.DLOC(6).AND.LPCTT.NE.0)CALL RSPEAK(127)
	GOTO 6030
...
6027	DTOTAL=DTOTAL+1
	...
	IF(RAN10(1000).LT.95*(DFLAG-2))STICK=STICK+1
6030	CONTINUE
...
2000	IF(LOC.EQ.0)GOTO 99
	...
	LPCTT=0
	IF(PCT(35))LPCTT=1
	IF(WZDARK.AND.LPCTT.NE.0)GOTO 90
	KK=RTEXT(16)
2001	IF(TOTING(BEAR))CALL RSPEAK(141)
	...
	LPCTT=0
	IF(PCT(25))LPCTT=1
	IF(LOC.EQ.33.AND.LPCTT.NE.0.AND..NOT.CLOSNG)CALL RSPEAK(8)
...
2608	FOOBAR=MIN0(0,-FOOBAR)
	IF(TURNS.EQ.0.AND.WD1.EQ.-27105441402 .AND.WD2.EQ.-27046820544 )CALL MAINT
	TURNS=TURNS+1
...
19999	K=43
	...
	IF(WD1.EQ.-31345457756 .AND.(WD2.EQ.-23804164734 .OR.WD2.EQ.-21736307292 ))
	1	GOTO 2010
	IF(WD1.EQ.-31345457756 .AND.WD2.NE.0)GOTO 2800
	IF((WD1.NE.-21736307292 .AND.WD1.NE.-25997991872 )
	1	.OR.(WD2.NE.-25448886616 .AND.WD2.NE.-31878294976 ))GOTO 2610
	IF(AT(VOCAB(WD2,1)))WD2=-25435647424 
2610	IF(WD1.NE.-21719559104 )GOTO 2630
	IWEST=IWEST+1
...
5010	IF(WD2.NE.0)GOTO 2800
	...
	CALL A5TOA1(WD1,WD1X,33958142016 ,TK,K)
	PRINT 5015,TRIM(WSTR(TK,K,1))
...
5190	IF((VERB.EQ.FIND.OR.VERB.EQ.INVENT).AND.WD2.EQ.0)GOTO 5010
	CALL A5TOA1(WD1,WD1X,-12457515684 ,TK,K)
	PRINT 5199,TRIM(WSTR(TK,K,1))
...
8000	CALL A5TOA1(WD1,WD1X,-21572291458 ,TK,K)
	PRINT 8002,TRIM(WSTR(TK,K,1))
8002	FORMAT(/A)
	OBJ=0
	GOTO 2600
...
9030	CALL A5TOA1(WD2,WD2X,18447605824 ,TK,K)
	IF(WD2.EQ.0)CALL A5TOA1(WD1,WD1X,18447605824 ,TK,K)
	IF(WD2.NE.0)WD1=WD2
	I=VOCAB(WD1,-1)
	IF(I.EQ.62.OR.I.EQ.65.OR.I.EQ.71.OR.I.EQ.2025)GOTO 9035
	PRINT 9032,TRIM(WSTR(TK,K,1))
9032	FORMAT(/'Okay, "',A)
	GOTO 2012
...
C  MOVE HIM THERE, TOO.  THEN DO A NULL MOTION TO GET NEW DESCRIPTION.
	...
	OBJ=0
	CALL GETIN(WD1,WD1X,WD2,WD2X)
	IF(WD1.NE.-20802691008 .AND.WD1.NE.-20645830592 )GOTO 2608
...
20210	CALL SPEAK(CTEXT(I))
	...
	K=CVAL(I)+1-SCORE
	KK=-6785327040 
	IF(K.EQ.1)KK=24831336512 
	PRINT 20212,K,WSTR(KK,1,2)
...
C  UNLESS BLKLIN IS FALSE.
	...
	IF(N.EQ.0)RETURN
	IF(LINES(N+1).EQ.33438965824 )RETURN
	IF(BLKLIN)PRINT 2
	K=N
1	L=IABS(LINES(K))-1
	K=K+1
...
C  WORD2 (CHARS 6 THRU 10 IN WORD2X), ELSE WORD2 IS SET TO ZERO.
	...
	LOGICAL BLKLIN
	COMMON /BLKCOM/ BLKLIN
	DIMENSION A(6),MASKS(6)
	DATA MASKS/536870912 ,4194304 ,32768 ,256 ,2 ,0/
	1	,BLANKS/17315143744 /

	IF(BLKLIN)PRINT 1
1	FORMAT()
2	CALL RDA5(A,4)
C3	FORMAT(4A5)		-- packed input, see RDA5
	J=0
	DO 9 I=1,4
	IF(A(I).NE.BLANKS)J=1
9	A(I)=A(I).AND.(SHIFT((A(I).AND.-34089189248 ),-1).XOR.-1)
...
C  THING WHICH NEEDS TO BE FIXED UP IS ABB(1).
	...
	CALL RDFRE(5,MBUF,1)
	HBEGIN=MBUF(1)
C1	FORMAT(G)		-- DEC free format, see RDFRE
	CALL MSPEAK(28)
	CALL RDFRE(5,MBUF,1)
	HEND=MBUF(1)
	CALL DATIME(D,T)
	...
	CALL MSPEAK(29)
	CALL RDA5(HNAME,4)
C2	FORMAT(4A5)		-- packed input, see RDA5
10	PRINT 12,SHORT
...
C  HBEGIN TO HEND.
	...
	PRINT 1
1	FORMAT()
	CALL HOURSX(WKDAY,-26911236006 ,17477233012 )
	CALL HOURSX(WKEND,-23748534182 ,17531858548 )
	CALL HOURSX(Holid,-29595637304 ,-16131704256 )
	CALL DATIME(D,T)
	IF(HEND.LT.D.OR.HEND.LT.HBEGIN)RETURN
	IF(HBEGIN.GT.D)GOTO 10
	PRINT 5,WSTR(HNAME,4,5)
5	FORMAT(/'Today is a holiday, namely ',A20)
	RETURN

10	D=HBEGIN-D
	T=-31801412776 
	IF(D.EQ.1)T=-31801430976 
	PRINT 15,D,WSTR(T,1,5),WSTR(HNAME,4,5)
...
C  USED BY HOURS (ABOVE) TO PRINT HOURS FOR EITHER WEEKDAYS OR WEEKENDS.
	...
	FROM=-1
	IF(H.NE.0)GOTO 10
	PRINT 2,WSTR(DAYS,2,5)
2	FORMAT(9X,A10,'  Open all day')
	RETURN
...
16	FORMAT(9X,A10,I4,':00 to',I3,':00')
18	FORMAT(19X,I4,':00 to',I3,':00')
	FIRST=.FALSE.
	FROM=TILL
	GOTO 10

20	IF(FIRST)PRINT 22,WSTR(DAYS,2,5)
22	FORMAT(9X,A10,'  Closed all day')
	RETURN
	END
...
C  CLOSED DUE TO PRIME TIME?  SEE HOURS (ABOVE) FOR DESC OF VARIABLES.
	...
	CALL MSPEAK(21)
	WKDAY=NEWHRX(-21584745528 ,-16131704256 )
	WKEND=NEWHRX(-21584745526 ,-9240462784 )
	HOLID=NEWHRX(-29595637304 ,-16131704256 )
	CALL MSPEAK(22)
	CALL HOURS
	RETURN
	END
...
C  INPUT PRIME TIME SPECS AND SET UP A WORD OF INTERNAL FORMAT.
	...
	DAYS(2)=DAY2
	NEWHRX=0
	PRINT 1,WSTR(DAYS,2,5)
1	FORMAT('Prime time on ',A10)
10	PRINT 2
2	FORMAT('from:')
	CALL RDFRE(5,NBUF,1)
	FROM=NBUF(1)
C3	FORMAT(G)		-- DEC free format, see RDFRE
...
57	MSG(M+I)=MOTBUF(I)
	K=MOTBUF(15)
C56	FORMAT(15A5)		-- packed input, see RDA5
	IF(K.EQ.17315143744 )GOTO 60
	CALL MSPEAK(24)
	GOTO 55
60	DO 62 I=1,14
	K=M+15-I
	IF(MSG(K).NE.17315143744 )GOTO 65
62	CONTINUE
	GOTO 90
65	MSG(M)=K+1
	M=K+1
...
C  FINAGLED DEC FUNCTIONS RETURN THE VALUES ONLY AS ASCII STRINGS!
	...
	DATA MONTHS/24471719514 ,24455070298 ,24484303450 ,24434463322 ,24484305242 ,24472374874 ,
	1	24472374362 ,24434624346 ,24509599834 ,24492758106 ,24488957530 ,24446681946 /
...
C  FUNCTION I2 TAKES 2-DIGIT ASCII AND YIELDS DECIMAL VALUE.
	...
	D=I2(DAT(1))-1
	X=((SHIFT(DAT(1),14).OR.SHIFT(DAT(2),-21)).AND..NOT.135274497 )
	1	.OR.24429740122 
...
C  ------------------------------------------------------------------
C  Generated by tools/convert.py -- do not edit.
C
C  The complete state of the game, for SUSPEND and for the images
C  MAGIC MODE leaves behind.  On TOPS-10 the monitor saved the whole
C  core image and every static variable came back; here every one of
C  them lives in a COMMON block and is written out by name.  MODE 1
C  writes, 0 reads, 2 counts.

	SUBROUTINE STATIO(LU,MODE)
	IMPLICIT INTEGER(A-Z)
	COMMON /TXTCOM/ RTEXT(205),LINES(9650)
	COMMON /BLKCOM/ BLKLIN
	COMMON /VOCCOM/ KTAB(300),ATAB(300),TABSIZ
	COMMON /PLACOM/ ATLOC(150),LINK(200),PLACE(100),FIXED(100),
	1	HOLDNG
	COMMON /MTXCOM/ MTEXT(35)
	COMMON /PTXCOM/ PTEXT(100)
	COMMON /ABBCOM/ ABB(150)
	COMMON /WIZCOM/ WKDAY,WKEND,HOLID,HBEGIN,HEND,HNAME(4),SHORT,
	1	MAGIC,MAGNM,LATNCY,SAVED,SAVET,SETUP
	COMMON /MOTCOM/ MSG(100)
	COMMON /RANCOM/ R
	COMMON /ADVSTA/ DSEEN(6),HINTED(20),TRAVEL(750),LTEXT(150),
	1	STEXT(150),KEY(150),COND(150),PLAC(100),FIXD(100),PROP(100),
	1	ACTSPK(35),CTEXT(12),CVAL(12),HINTLC(20),HINTS(80),TK(20),
	1	DLOC(6),ODLOC(6),IDONDX,WZDARK,LMWARN,CLOSNG,PANIC,CLOSED,
	1	GAVEUP,SCORNG,DEMO,YEA,LINSIZ,TRVSIZ,LOCSIZ,VRBSIZ,RTXSIZ,
	1	CLSMAX,HNTSIZ,MAGSIZ,I,LINUSE,TRVS,CLSSES,SECT,OLDLOC,LOC,J,
	1	KK,K,NEWLOC,L,TABNDX,OBJ,VERB,HNTMAX,MAXTRS,TALLY,TALLY2,
	1	KEYS,LAMP,GRATE,CAGE,ROD,ROD2,STEPS,BIRD,DOOR,PILLOW,SNAKE,
	1	FISSUR,TABLET,CLAM,OYSTER,MAGZIN,DWARF,KNIFE,FOOD,BOTTLE,
	1	WATER,OIL,PLANT,PLANT2,AXE,MIRROR,DRAGON,CHASM,TROLL,TROLL2,
	1	BEAR,MESSAG,VEND,BATTER,NUGGET,COINS,CHEST,EGGS,TRIDNT,VASE,
	1	EMRALD,PYRAM,PEARL,RUG,CHAIN,BACK,LOOK,CAVE,NULL,ENTRNC,
	1	DPRSSN,SAY,LOCK,THROW,FIND,INVENT,CHLOC,CHLOC2,DFLAG,DALTLC,
	1	TURNS,IWEST,KNFLOC,DETAIL,ABBNUM,MAXDIE,NUMDIE,DKILL,FOOBAR,
	1	BONUS,CLOCK1,CLOCK2,LIMIT,DTOTAL,ATTACK,STICK,OLDLC2,SPK,
	1	HINT,WD1,WD1X,WD2,WD2X,KQ,LL,SPICES,K2,SCORE,MXSCOR,FOO

C  /TXTCOM/
	CALL BLKIO(LU,MODE,RTEXT,205)
	CALL BLKIO(LU,MODE,LINES,9650)
C  /BLKCOM/
	CALL BLKIO(LU,MODE,BLKLIN,1)
C  /VOCCOM/
	CALL BLKIO(LU,MODE,KTAB,300)
	CALL BLKIO(LU,MODE,ATAB,300)
	CALL BLKIO(LU,MODE,TABSIZ,1)
C  /PLACOM/
	CALL BLKIO(LU,MODE,ATLOC,150)
	CALL BLKIO(LU,MODE,LINK,200)
	CALL BLKIO(LU,MODE,PLACE,100)
	CALL BLKIO(LU,MODE,FIXED,100)
	CALL BLKIO(LU,MODE,HOLDNG,1)
C  /MTXCOM/
	CALL BLKIO(LU,MODE,MTEXT,35)
C  /PTXCOM/
	CALL BLKIO(LU,MODE,PTEXT,100)
C  /ABBCOM/
	CALL BLKIO(LU,MODE,ABB,150)
C  /WIZCOM/
	CALL BLKIO(LU,MODE,WKDAY,1)
	CALL BLKIO(LU,MODE,WKEND,1)
	CALL BLKIO(LU,MODE,HOLID,1)
	CALL BLKIO(LU,MODE,HBEGIN,1)
	CALL BLKIO(LU,MODE,HEND,1)
	CALL BLKIO(LU,MODE,HNAME,4)
	CALL BLKIO(LU,MODE,SHORT,1)
	CALL BLKIO(LU,MODE,MAGIC,1)
	CALL BLKIO(LU,MODE,MAGNM,1)
	CALL BLKIO(LU,MODE,LATNCY,1)
	CALL BLKIO(LU,MODE,SAVED,1)
	CALL BLKIO(LU,MODE,SAVET,1)
	CALL BLKIO(LU,MODE,SETUP,1)
C  /MOTCOM/
	CALL BLKIO(LU,MODE,MSG,100)
C  /RANCOM/
	CALL BLKIO(LU,MODE,R,1)
C  /ADVSTA/
	CALL BLKIO(LU,MODE,DSEEN,6)
	CALL BLKIO(LU,MODE,HINTED,20)
	CALL BLKIO(LU,MODE,TRAVEL,750)
	CALL BLKIO(LU,MODE,LTEXT,150)
	CALL BLKIO(LU,MODE,STEXT,150)
	CALL BLKIO(LU,MODE,KEY,150)
	CALL BLKIO(LU,MODE,COND,150)
	CALL BLKIO(LU,MODE,PLAC,100)
	CALL BLKIO(LU,MODE,FIXD,100)
	CALL BLKIO(LU,MODE,PROP,100)
	CALL BLKIO(LU,MODE,ACTSPK,35)
	CALL BLKIO(LU,MODE,CTEXT,12)
	CALL BLKIO(LU,MODE,CVAL,12)
	CALL BLKIO(LU,MODE,HINTLC,20)
	CALL BLKIO(LU,MODE,HINTS,80)
	CALL BLKIO(LU,MODE,TK,20)
	CALL BLKIO(LU,MODE,DLOC,6)
	CALL BLKIO(LU,MODE,ODLOC,6)
	CALL BLKIO(LU,MODE,IDONDX,1)
	CALL BLKIO(LU,MODE,WZDARK,1)
	CALL BLKIO(LU,MODE,LMWARN,1)
	CALL BLKIO(LU,MODE,CLOSNG,1)
	CALL BLKIO(LU,MODE,PANIC,1)
	CALL BLKIO(LU,MODE,CLOSED,1)
	CALL BLKIO(LU,MODE,GAVEUP,1)
	CALL BLKIO(LU,MODE,SCORNG,1)
	CALL BLKIO(LU,MODE,DEMO,1)
	CALL BLKIO(LU,MODE,YEA,1)
	CALL BLKIO(LU,MODE,LINSIZ,1)
	CALL BLKIO(LU,MODE,TRVSIZ,1)
	CALL BLKIO(LU,MODE,LOCSIZ,1)
	CALL BLKIO(LU,MODE,VRBSIZ,1)
	CALL BLKIO(LU,MODE,RTXSIZ,1)
	CALL BLKIO(LU,MODE,CLSMAX,1)
	CALL BLKIO(LU,MODE,HNTSIZ,1)
	CALL BLKIO(LU,MODE,MAGSIZ,1)
	CALL BLKIO(LU,MODE,I,1)
	CALL BLKIO(LU,MODE,LINUSE,1)
	CALL BLKIO(LU,MODE,TRVS,1)
	CALL BLKIO(LU,MODE,CLSSES,1)
	CALL BLKIO(LU,MODE,SECT,1)
	CALL BLKIO(LU,MODE,OLDLOC,1)
	CALL BLKIO(LU,MODE,LOC,1)
	CALL BLKIO(LU,MODE,J,1)
	CALL BLKIO(LU,MODE,KK,1)
	CALL BLKIO(LU,MODE,K,1)
	CALL BLKIO(LU,MODE,NEWLOC,1)
	CALL BLKIO(LU,MODE,L,1)
	CALL BLKIO(LU,MODE,TABNDX,1)
	CALL BLKIO(LU,MODE,OBJ,1)
	CALL BLKIO(LU,MODE,VERB,1)
	CALL BLKIO(LU,MODE,HNTMAX,1)
	CALL BLKIO(LU,MODE,MAXTRS,1)
	CALL BLKIO(LU,MODE,TALLY,1)
	CALL BLKIO(LU,MODE,TALLY2,1)
	CALL BLKIO(LU,MODE,KEYS,1)
	CALL BLKIO(LU,MODE,LAMP,1)
	CALL BLKIO(LU,MODE,GRATE,1)
	CALL BLKIO(LU,MODE,CAGE,1)
	CALL BLKIO(LU,MODE,ROD,1)
	CALL BLKIO(LU,MODE,ROD2,1)
	CALL BLKIO(LU,MODE,STEPS,1)
	CALL BLKIO(LU,MODE,BIRD,1)
	CALL BLKIO(LU,MODE,DOOR,1)
	CALL BLKIO(LU,MODE,PILLOW,1)
	CALL BLKIO(LU,MODE,SNAKE,1)
	CALL BLKIO(LU,MODE,FISSUR,1)
	CALL BLKIO(LU,MODE,TABLET,1)
	CALL BLKIO(LU,MODE,CLAM,1)
	CALL BLKIO(LU,MODE,OYSTER,1)
	CALL BLKIO(LU,MODE,MAGZIN,1)
	CALL BLKIO(LU,MODE,DWARF,1)
	CALL BLKIO(LU,MODE,KNIFE,1)
	CALL BLKIO(LU,MODE,FOOD,1)
	CALL BLKIO(LU,MODE,BOTTLE,1)
	CALL BLKIO(LU,MODE,WATER,1)
	CALL BLKIO(LU,MODE,OIL,1)
	CALL BLKIO(LU,MODE,PLANT,1)
	CALL BLKIO(LU,MODE,PLANT2,1)
	CALL BLKIO(LU,MODE,AXE,1)
	CALL BLKIO(LU,MODE,MIRROR,1)
	CALL BLKIO(LU,MODE,DRAGON,1)
	CALL BLKIO(LU,MODE,CHASM,1)
	CALL BLKIO(LU,MODE,TROLL,1)
	CALL BLKIO(LU,MODE,TROLL2,1)
	CALL BLKIO(LU,MODE,BEAR,1)
	CALL BLKIO(LU,MODE,MESSAG,1)
	CALL BLKIO(LU,MODE,VEND,1)
	CALL BLKIO(LU,MODE,BATTER,1)
	CALL BLKIO(LU,MODE,NUGGET,1)
	CALL BLKIO(LU,MODE,COINS,1)
	CALL BLKIO(LU,MODE,CHEST,1)
	CALL BLKIO(LU,MODE,EGGS,1)
	CALL BLKIO(LU,MODE,TRIDNT,1)
	CALL BLKIO(LU,MODE,VASE,1)
	CALL BLKIO(LU,MODE,EMRALD,1)
	CALL BLKIO(LU,MODE,PYRAM,1)
	CALL BLKIO(LU,MODE,PEARL,1)
	CALL BLKIO(LU,MODE,RUG,1)
	CALL BLKIO(LU,MODE,CHAIN,1)
	CALL BLKIO(LU,MODE,BACK,1)
	CALL BLKIO(LU,MODE,LOOK,1)
	CALL BLKIO(LU,MODE,CAVE,1)
	CALL BLKIO(LU,MODE,NULL,1)
	CALL BLKIO(LU,MODE,ENTRNC,1)
	CALL BLKIO(LU,MODE,DPRSSN,1)
	CALL BLKIO(LU,MODE,SAY,1)
	CALL BLKIO(LU,MODE,LOCK,1)
	CALL BLKIO(LU,MODE,THROW,1)
	CALL BLKIO(LU,MODE,FIND,1)
	CALL BLKIO(LU,MODE,INVENT,1)
	CALL BLKIO(LU,MODE,CHLOC,1)
	CALL BLKIO(LU,MODE,CHLOC2,1)
	CALL BLKIO(LU,MODE,DFLAG,1)
	CALL BLKIO(LU,MODE,DALTLC,1)
	CALL BLKIO(LU,MODE,TURNS,1)
	CALL BLKIO(LU,MODE,IWEST,1)
	CALL BLKIO(LU,MODE,KNFLOC,1)
	CALL BLKIO(LU,MODE,DETAIL,1)
	CALL BLKIO(LU,MODE,ABBNUM,1)
	CALL BLKIO(LU,MODE,MAXDIE,1)
	CALL BLKIO(LU,MODE,NUMDIE,1)
	CALL BLKIO(LU,MODE,DKILL,1)
	CALL BLKIO(LU,MODE,FOOBAR,1)
	CALL BLKIO(LU,MODE,BONUS,1)
	CALL BLKIO(LU,MODE,CLOCK1,1)
	CALL BLKIO(LU,MODE,CLOCK2,1)
	CALL BLKIO(LU,MODE,LIMIT,1)
	CALL BLKIO(LU,MODE,DTOTAL,1)
	CALL BLKIO(LU,MODE,ATTACK,1)
	CALL BLKIO(LU,MODE,STICK,1)
	CALL BLKIO(LU,MODE,OLDLC2,1)
	CALL BLKIO(LU,MODE,SPK,1)
	CALL BLKIO(LU,MODE,HINT,1)
	CALL BLKIO(LU,MODE,WD1,1)
	CALL BLKIO(LU,MODE,WD1X,1)
	CALL BLKIO(LU,MODE,WD2,1)
	CALL BLKIO(LU,MODE,WD2X,1)
	CALL BLKIO(LU,MODE,KQ,1)
	CALL BLKIO(LU,MODE,LL,1)
	CALL BLKIO(LU,MODE,SPICES,1)
	CALL BLKIO(LU,MODE,K2,1)
	CALL BLKIO(LU,MODE,SCORE,1)
	CALL BLKIO(LU,MODE,MXSCOR,1)
	CALL BLKIO(LU,MODE,FOO,1)
	RETURN
	END