...
static void goback()
{
	...
	/* PORT: only a destination that is a location (Woods: "IF(LL.GT.300)
	   GOTO 22"); gettrav() of a special move or a message read cave[]
	   far past its end, and RETREAT crashed */
	if (!travel[kk].tcond && travel[kk].tdest <= MAXLOC) {
	    temp = travel[kk].tdest;
	    gettrav(temp, strav);
	    if (forced(temp) && strav[0].tdest == want)
		k2 = kk;	/* PORT: was temp, a location number used
				   below as an index into travel[] (far past
				   its end: RETREAT crashed); Pohl's kk */
	}
    }
    if (k2 >= 0) {
