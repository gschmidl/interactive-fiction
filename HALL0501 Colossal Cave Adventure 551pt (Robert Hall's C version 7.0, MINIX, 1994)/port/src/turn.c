...
void turn()
{
	...
	/* See if a dwarf has seen him and has come from where he wants
	   to go. */
	/* PORT: Fix 1 - "g.loc_attrib[g.loc] & NOPIRAT == 0" is
	   loc_attrib & (NOPIRAT == 0), always 0, so no dwarf ever blocked
	   the way (Pohl's bug, carried over); --no-fixes keeps that */
	if (g.newloc != g.loc && !forced(g.loc) && port_fixes &&
	    (g.loc_attrib[g.loc] & NOPIRAT) == 0)
