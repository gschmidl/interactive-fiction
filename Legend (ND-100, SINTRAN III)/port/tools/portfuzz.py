"""Play random games of LEGEND on the port and look for trouble.
...
commands (default 8 seeds x 200), on a scratch copy of data\\ with --raw
--no-hold and a fixed clock, under a time limit.  Printed as trouble: a run
that hangs or does not end, an unimplemented monitor call or instruction, a
BASIC run-time error ("** Error"), or a prompt the player does not know.
With --keys the typing of each game is kept (DIR\\portNN.keys).  --original
plays build\\original\\LEGEND-LU.PROG, the recovered source as it was.
"""
import os
import re
import shutil
...
ORIGINAL = os.path.join(PORT, 'build', 'original', 'LEGEND-LU.PROG')
...
def play(seed, commands, work, keep=None, first=None, original=False):
    args = [EXE, '--data', work, '--raw', '--no-hold', '-Z', str(1789668000 + seed * 60)]
    if original:
        args += ['--prog', ORIGINAL]
...
def main():
    ...
    for seed in range(1, seeds + 1):
        work = tempfile.mkdtemp(prefix='legendfuzz')
        try:
