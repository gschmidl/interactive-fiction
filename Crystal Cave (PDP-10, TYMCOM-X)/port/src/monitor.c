/* monitor.c -- the slice of the TOPS-10 / TYMCOM-X monitor that CRYSTAL CAVE
 * needs.  Taken from the EXPLOR port, which needs the same slice: both are
 * DEC FORTRAN programs off the same Tymshare tapes, and they ask the
 * monitor for the same things.  Only the default core file name differs.
 *
...
const char *opt_corefile = "cave.core";
int  opt_autosave = 1;
