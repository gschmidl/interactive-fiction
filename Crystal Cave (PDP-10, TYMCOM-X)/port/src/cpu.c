...
void cpu_reset(void)
{
    ...
    /* The loader, not the SAVE file, supplies these: .JBREL is the
     * highest legal low segment address, and FOROTS clears core up to
     * it before it does anything else. */
    if (RH(M[0133]))
        M[044] = XWD(0, RH(M[0133]));
    else
        M[044] = XWD(0, ((RH(M[0121]) + 1 + 01777) & ~01777) - 1);
