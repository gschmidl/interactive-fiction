...
5	  CALL FORMAT(0,'There are numerous dungeons and passages
     ...
     + adventuring are:!/!/')
	  ...
	  J=0
	  DO WHILE(.TRUE.)
	    READ(21,FMT='(A37)',ERR=2) DUNNAM(I)
...
*****
	...
	OPEN(UNIT=24,FILE='BSU$USER_2:[00CKKELLE.QUEST]DUNGEON.DTA',
     +STATUS='OLD',ACCESS='DIRECT',ORGANIZATION='RELATIVE',RECL=4,FORM=
     +'FORMATTED',CARRIAGECONTROL='LIST',READONLY)
...
*****
	...
	OPEN(UNIT=21,FILE='BSU$USER_2:[00CKKELLE.QUEST]DUNNAM.DTA',
...
3	IF(DICE(1,100).LE.I.AND.LOCATION.NE.23)THEN
	  ...
	  IF(DELAY.NE.0)CALL SLEEP(DELAY)
	  CALL TREASURE(I)
	  CALL ARMOR
	  CALL ADDGOLD(I)
	  IF(LIFE.EQ.0)GOTO 999
...
2	FORMAT(I4)
	...
	K=POINTER(J)
	READ(24'K,2) LEVELLENGTH
	READ(24'K+1,2) LEVELWIDTH
	K=K+2

	DO L=1,LEVELLENGTH
	  DO L1=1,LEVELWIDTH
	    READ(24'K,2) MAP(L,L1)
	    K=K+1
	  ENDDO
	ENDDO

	READ(24'K,2) STAIRSUPX
	READ(24'K+1,2) STAIRSUPY
	READ(24'K+2,2) STAIRSDOWNX
	READ(24'K+3,2) STAIRSDOWNY
...
*****
	...
	OPEN(UNIT=22,FILE='BSU$USER_2:[00CKKELLE.QUEST]MON.DTA',
     +STATUS='OLD',FORM='FORMATTED',CARRIAGECONTROL='LIST',READONLY)
	I=1
	DO WHILE(.TRUE.)
	  READ(22,1,ERR=2) MONSTER(I),MONSTERSTAT(I,1),MONSTERSTAT(I,2)
1	  FORMAT(A20,I8,I6)
	  I=I+1
	ENDDO
2	CALL CLOSEFILE(22)
