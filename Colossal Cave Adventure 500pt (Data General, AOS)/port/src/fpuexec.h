    ...
    }
{
    ...
    case F_FTE: case F_FTD: case F_FCLE:    /* trap enable/disable/clear  */
        FPSR = 0; return 1;
    /* FPSH/FPOP (S/140 Programmer's Reference 4-60): an 18-word block --
     * FPSR bits 0-15, FPSR bits 16-31, then FAC0..FAC3 four words each, in
     * that order upwards from the old stack top.  The Z and N condition
     * bits (FPSR bits 6 and 7) travel in the first word.  A routine can
     * change an FPAC between the two, and the caller's value -- DG.PR's
     * random number state lives in them -- must come back. */
    case F_FPSH: {
        int k, s;
        word hi = (word)(((FPSR >> 16) & 0xFCFF) | (fp_z ? 0x0200 : 0) | (fp_n ? 0x0100 : 0));
        push(hi);
        push((word)FPSR);
        for (k = 0; k < 4; k++)
            for (s = 48; s >= 0; s -= 16) push((word)(FPAC[k] >> s));
        stack_check(PC);
        return 1;
    }
    case F_FPOP: {
        ...
        for (k = 3; k >= 0; k--) {
            uint64_t v = 0;
            for (s = 0; s <= 48; s += 16) v |= (uint64_t)pop() << s;
            FPAC[k] = v;
        }
        lo = pop();
        hi = pop();
        fp_z = (hi & 0x0200) != 0;
        fp_n = (hi & 0x0100) != 0;
        FPSR = ((uint64_t)(hi & 0xFCFF) << 16) | lo;
        stack_check_under(PC);
        return 1;
    }
    }
    if (skip >= 0) {
