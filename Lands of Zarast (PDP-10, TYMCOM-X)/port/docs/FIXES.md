...
| nothing thrown can touch the stirge | THROW picks its target with `HP(I)>2` (26100) and attacks only if `DAM(thrown)>2 AND HP(I)>2` (26200). Objects have 1 hit point, so "more than 2" was meant as "a monster", but the stirge has 2 | The stirge is a target for anything thrown that can attack at all. Burning oil (25120) still passes it by. In the store the merchant, noun 5, is still the first target, as before |
