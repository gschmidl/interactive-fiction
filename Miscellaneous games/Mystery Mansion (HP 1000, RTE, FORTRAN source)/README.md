...
**Status: PORTED 2026-09-21 (first pass).** `port\run.bat` runs `port\mmm.exe`, the game compiled from its own
FTN4 source; see `port\README.md`. Revision 9, compiled only, RUNS since 2026-09-22 on the site's own RTE-IVB system
in SIMH: `port\rev9\run.bat`, see `port\rev9\README.md`.
...
revision 16, runs as the original program under our MPE emulator in `..\..\HP-3000 games\Mystery Mansion (HP 3000, MPE)`; the two
differ in wording here and there.

Sources (verified copies, md5, in `archive_original`):

- `bitsavers.org/bits/HP/HP_1000_software_collection/specials/CSL-1000_Startup-Tape.zip` - the CSL/1000 startup tape of
...
- `bitsavers.org/bits/HP/Crisis_Computer_Tapes/ccc_9trkTapes_20050826/f1_dskup_f2_rte2250sys.tap.gz` - an RTE system backup
  ...
  (2026-09-22) on the site's own RTE-IVB system in SIMH: `port\rev9\run.bat`, see `port\rev9\README.md`.
