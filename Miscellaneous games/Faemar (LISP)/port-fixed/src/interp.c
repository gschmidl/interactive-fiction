/*
 * FAEMAR (FIXED BUILD) -- a small dynamically-scoped Lisp interpreter that
 * runs the 1982 Faemar adventure source (game.lsp engine + faemar world
 * data), compiled to a native Windows executable.
 *
 ...
 * ~70 primitives) to interpret the archived source text, embedded in
 * embedded_sources.h. This build's embedded world data (see faemar_fixed.txt)
 * carries two deliberate, documented departures from the faithful port in
 * ../port/: the Hand's "move X on hand" wielding bug is fixed to behave
 * like "to hand"/"in hand", and taking the idol's eyes now prints a win
 * message and ends the game (see the added HALT primitive below). The
 * unmodified, bug-for-bug-faithful build lives in ../port/ untouched.
 */
...
/* HALT: exits immediately with no extra message, unlike EXIT (which prints
   "Goodbye!" via do_quit()). Used by the FIXED build's win-condition
   handler on the idol's eyes, which prints its own message beforehand. */
static Node *n_halt(Symbol *s, Node **a, int n) { (void)s;(void)a;(void)n; fflush(stdout); exit(0); return NIL; }
