 ...
 * 1972), checked against its worked examples; see docs/ISA_NOTES.md.  The
 * IV/90 extensions (op codes 707-767) stop the run for now. */
#include <stdarg.h>
#include <string.h>
#include "fp4.h"
...
word screen_base = 0140;
void (*scroll_hook)(void);

static void mve(int c)
{
    int i;

    /* the first move of a scroll: line 1 onto line 0 */
    if (scroll_hook && X3 == 077777740u && ((X2 + 1) & A15) == ((screen_base + 040) & A15))
        scroll_hook();
    for (i = 0; i <= c; i++) {
        X2 = (X2 + 1) & W24;
        wr(X2 + X3, rd(X2));
