/* cpu.c -- DECsystem-10 processor emulation.
 *
 * Enough of the architecture to run a FORTRAN-10 program on top of
 * FOROTS: the full non-privileged instruction set, single and double
 * precision floating point, byte instructions, and both call gates the
 * program uses -- TOPS-10 UUOs and the TOPS-20 JSYS.
 */
#include <stdlib.h>
#include <stdarg.h>
#include <string.h>
...
/* effective address                                                    */
/* ------------------------------------------------------------------ */
/* ADJBP -- move a byte pointer by a signed number of bytes.
 *
 * The pointer is P,,S,,Y with P the bit position of the low end of the
 * byte and S its size, and the bytes of a word are numbered 0 upwards
 * from the left.  Count which byte the pointer is on, add the
 * adjustment, then split the answer back into a word displacement and a
 * byte within the word -- flooring, so that stepping backwards past the
 * start of a word lands on the last byte of the one before.
 *
 * A byte size of zero has no bytes to count, so the pointer comes back
 * unchanged, which is what the hardware does.
 */
...
static w36 adjbp(w36 count, w36 ptr)
{
    int p = (int)((ptr >> 30) & 077);
    int s = (int)((ptr >> 24) & 077);
    long long a = sx36(count);
    long long per, cur, want, disp, pos;

    if (s == 0 || s > 36) return ptr;
    per  = 36 / s;                        /* bytes in a word           */
    cur  = per - (p / s);                 /* which byte we are on      */
    want = cur + a;
    disp = floordiv(want - 1, per);
    pos  = want - 1 - disp * per;
    return (ptr & ~(((w36)077 << 30) | (w36)HMASK))
         | ((w36)(36 - s * (pos + 1)) << 30)
         | (((w36)(RH(ptr) + disp)) & HMASK);
}
...
void cpu_reset(void)
{
    ...
    /* .JBREL, .JBSA and .JBFF come out of the EXE as the TOPS-20 loader
     * left them; nothing here has to invent them.  Location 41 holds the
     * game's own LUUO trap (a HALT -- FORTRAN-10 does not use LUUOs). */
    PC = image_start;
    FLAGS = F_USER;
    halted = 0;
    exit_code = 0;
...
static void execute(w36 inst, int depth)
{
    ...
    if (op == 0104) {                       /* JSYS -- the TOPS-20 side */
        int skip = do_jsys(e);
        PC = (PC + skip) & HMASK;
        return;
    }
