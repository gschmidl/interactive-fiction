"""The game at a real console -- the way play.bat is played.
    ...
    python tests/consoleplay.py [adventure4.exe]
...
port (saved games go to its saves\).  Checked on a model of the screen:

  1. "Would you like instructions?" is answered on the line below it, and
     "? " waits with the cursor after it (IOA$ '? %$': no new line).
  2. A command typed there shows once, on the prompt's line: the console
     echoes it (line input), the program does not.
  3. QUIT, Y: the program ends by itself, exit code 0.
"""
import ctypes
...
EXE = os.path.join(ROOT, 'adventure4.exe')
CWD = ROOT
...
def prompt(s):
    return s.row(s.r) == '?' and s.c == 2


def main():
    global CWD
    exe = sys.argv[1] if len(sys.argv) > 1 else EXE
    CWD = tempfile.mkdtemp(prefix='adv4con')
    for f in ('ADVINIT1.DAT', 'ADVINIT2.DAT', 'ADVINIT3.DAT', 'ADVINIT4.DAT'):
        shutil.copy(os.path.join(os.path.dirname(exe), f), CWD)
    shutil.copy(exe, os.path.join(CWD, 'adventure4.exe'))
    w = Window('adventure4', [os.path.join(CWD, 'adventure4.exe')])
    try:
        w.wait_for('Would you like instructions?')
        w.wait_screen(lambda s: s.row(s.r - 1) == 'Would you like instructions?'
                      and s.c == 0)
        w.type('no\r')
        w.wait_for('wide path leads northwest.')
        w.wait_screen(prompt)
        w.type('in\r')
        w.wait_for('There is a shiny brass lamp nearby.')
        w.wait_screen(prompt)
        rows = [w.screen.row(k) for k in range(w.screen.rows)]
        if '? in' not in rows or 'no' not in rows:
            raise AssertionError('the answers are not where they belong:\n'
                                 + w.screen.text())
        if any(r.startswith('? inin') for r in rows):
            raise AssertionError('typing shows twice:\n' + w.screen.text())
        w.type('quit\r')
