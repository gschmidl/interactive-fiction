/* monitor.c -- the slice of the TOPS-10 monitor that BLOCKE needs.
 *
...
/* DEVCHR as reported for TTY.  FOROTS reads this closely: the left half
 * says the device is a terminal (020000), is available (000040) and can
 * do both input and output (000003); the right half is the bit mask of
 * legal data modes, here ASCII and ASCII-line.  Without the terminal bit
 * FOROTS drives the TTY with buffered IN/OUT instead of TTCALL and never
 * sees a typed line. */
w36  opt_devchr_tty = 0020043000003ULL;
int  opt_faketime = -1;          /* minutes past midnight, or -1       */
const char *opt_corefile = "archon.core";
int  opt_autosave = 1;
...
/* The game's SUSPEND command tells the player to break out and save the
 * core image; watch the output stream for it so that the driver knows to
 * write one out when the program stops. */
int suspend_requested = 0;
static char otail[64];
static int otailn;
