...
C     NXTCHR/INP2 must survive between separate calls to GETIN (that's
C     how it resumes a multi-command line, and how "AGAIN" repeats the
C     last one) -- a plain local without SAVE is only guaranteed to
C     keep its value across calls if it's DATA-initialized (like the
C     ones above) or explicitly SAVEd, so make that explicit here.
C     (Cave's comblk.fi puts NXTCHR in COMMON instead, which already
C     guarantees it; Adventure's comblk.fi doesn't have it.)
      SAVE NXTCHR, INP2
