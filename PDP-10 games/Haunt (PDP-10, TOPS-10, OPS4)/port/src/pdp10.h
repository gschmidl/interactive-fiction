/* pdp10.h -- 36-bit DECsystem-10 CPU emulation for the EXPLOR port. */
#ifndef PDP10_H
#define PDP10_H
...
/* Core snapshots (main.c).  Haunt itself has no save command -- none
 * existed in the original -- so this is the only save it can have. */
int  save_core(const char *path);
int  load_core(const char *path);
extern const char *opt_corefile;
