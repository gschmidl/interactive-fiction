...
| no way to get gold, and a treasure scores again every time it is dropped in the store | Gold is set once (line 1600), and only BUY changes it. A treasure scores half its value when picked up (11320) and its full value when dropped in the store (11720), and nothing records that it was already deposited, so picking it up and dropping it again scores each time | A treasure dropped in the store is sold: it leaves the game and pays its value in gold. The score is still counted as before. Two kinds stay on the floor with the usual "OK.". The *magic* wand is not sold, because the game is won with it, and its value is zeroed after it first scores in the store, so it scores only once. A treasure with a negative value (the chalice or the decanter) is not bought. It lowers the score by its value while it lies in the store: dropping it there takes the points off, and picking it up again gives them back. Picking it up or dropping it anywhere else neither costs nor gains anything. The mace, the axe and the club are sold the same way but not used up: the merchant puts them in stock, the price list shows them while he has them, and BUY sells them back for twice what he paid (mace 20, axe 48, club 50). They score as treasures only until their first sale. Bought back, they give no score when picked up or sold again, but still pay gold. The store's own goods that are not used up (scimitar, spear, sword, armor, shield, and a lamp that has never burned for a move) can be returned the same way: the merchant pays half the listed price, rounded down, and puts them back in stock at the full price. Torches, oil and a burned lamp stay on the floor. The star crystal is never sold, because the game is won with it. All of this needs the merchant alive. KILL refuses him, but a thrown weapon can kill him, and then BUY answers "BUY FROM WHOM?" and a drop in the store is an ordinary drop. A treasure dropped there still scores as in the original, once. Selling is the one new rule, not a repair of an existing one |
...
| monsters hit for negative damage, which adds strength | A monster's blow (21940) is its damage roll less its rating, and it is never clamped. The orc, zombie, goblin, scorpion, kobold, stirge, su-monster, frog, wereboar, weretiger, werewolf, wererat, cobra and newt can roll below 0. The player's own blows are clamped at 21060 | A negative blow does 0 damage (line 21980) |
| neither burning oil nor anything thrown can touch the stirge | A noun with fewer than 2 hit points is an object, and the monster loop (17220) skips it, so 2 is the fewest a monster can have. The stirge has 2. But oil picks its victims with `HPMAX(I)>2` (25120), and THROW picks its target with `HP(I)>2` (26100) and attacks only if `DAM(thrown)>2 AND HP(I)>2` (26200). "More than 2" was meant as "a monster" | The stirge keeps its 2 hit points and is a victim of burning oil and a target for anything thrown that can attack at all. Giving it 1 hit point instead would make it an object that never moves or attacks, and still out of reach of both. In the store the merchant, noun 5, is still the first target of a throw, as before |
