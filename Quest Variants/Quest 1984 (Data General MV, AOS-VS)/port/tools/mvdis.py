...
def find(ir):
    # The same special cases as mvfind() in ../src32/mvops.h, in the same
    # order, so a listing here decodes exactly as the emulator executes.
    if (ir & 0x843F) == 0x8038: return (0x8038, 'WBR', 0x23, 1)
    if ir == 0xCF09: return (0xC709, 'XVCT', 0x05, 1)
    if ir == 0xCF19: return (0xC719, 'QSCAN', 0x05, 1)
    if (ir & 0x87FF) == 0x8569:
        return (0x8569, 'WCLM', 0x0F, 5 if ((ir >> 13) & 3) == ((ir >> 11) & 3) else 1)
    if (ir & 0x8FCF) == 0x8F49: return (0x8F49, 'WSKBO', 0x24, 1)
    if (ir & 0x8FCF) == 0x8F89: return (0x8F89, 'WSKBZ', 0x24, 1)
    for mk in MASKS:
        b = ir & mk
...
def operands(ir, pc, M, o):
    ...
    ixn = ['', ',PC', ',2', ',3'][idx]
    if ty == 0x23:
        d = (((ir >> 11) & 0xF) << 4) | (((ir >> 8) & 3) << 2) | ((ir >> 6) & 3)
        if d & 0x80: d -= 0x100
        return '%+d   ;%06X' % (d, (pc + d) & 0xFFFFFF)
    if ty == 0x05 and ln == 1: return ''
    ...
    if ty in (0x2E,0x26,0x27,0x28,0x13,0x14,0x15,0x1A,0x2B,0x2C,0x18,0x19,0x06,0x07,0x04,0x03,0x02):
        ...
        tgt = ''
        if idx == 1: tgt = '   ;%06X' % ((pc + 1 + d) & 0xFFFFFF)
        elif idx == 0: tgt = '   ;%06X' % (d & 0xFFFFFF)
        if ty in (0x0F,): pass
    ...
    if ty == 0x16:
        t = (M[pc+1] << 16) | M[pc+2]
        return '%06X,%d' % (t & 0xFFFFFF, M[pc+3])
    if ty == 0x29:
        d = M[pc+1]; d = d - 0x10000 if d & 0x8000 else d
        return '%06X,%d' % ((pc + 1 + d) & 0xFFFFFF, M[pc+2])
    return ''
