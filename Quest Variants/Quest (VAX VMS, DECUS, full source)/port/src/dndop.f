...
5     FORMAT(' Old location: ',I4)
      ...
      CALL TTYREC(ICHAR('$'),' New location: ')
      CALL INPUTNUMBER(K)
      WRITE(21,3,REC=N) K
      GOTO 6
      END
...
*****
      ...
      CHARACTER NORTH(8)*7,NAMES(6)*20,QFN*256
...
1     CALL FORMAT(4,'Dungeon number: ')
      CALL INPUTNUMBER(J)
      IF(J.LT.1.OR.J.GT.6)RETURN
      CALL QPATH('mapped.dat',QFN)
      OPEN(UNIT=22,FILE=QFN,STATUS='UNKNOWN')
...
*****
      ...
      K=POINTER(I)
      READ(21,1,REC=K) LEVELLENGTH
      READ(21,1,REC=K+1) LEVELWIDTH
1     FORMAT(I4)

      K=K+2
      DO 2 J=1,LEVELLENGTH
        DO 3 J1=1,LEVELWIDTH
          READ(21,1,REC=K) MAP(J,J1)
          K=K+1
3       CONTINUE
2     CONTINUE

      READ(21,1,REC=K) STAIRSUPX
      READ(21,1,REC=K+1) STAIRSUPY
      READ(21,1,REC=K+2) STAIRSDOWNX
      READ(21,1,REC=K+3) STAIRSDOWNY
...
*****
      ...
      CALL QPATH('dungeon.dta',QFN)
      OPEN(UNIT=21,FILE=QFN,STATUS='OLD',ACCESS='DIRECT',
     +FORM='FORMATTED',RECL=4,ACTION='READ')
      RETURN
