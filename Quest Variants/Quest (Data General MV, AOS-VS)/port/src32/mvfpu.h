...
static void fp_exec32(word ir, dword at, const mvop *m)
{
    ...
    case 0x84D8: FPAC[f2] = fp_single(FPAC[f1]);                 /* FRDS */
                 fp_setcc(FPAC[f2]); break;
    /* 103750 -- EBID.SR's FLOGD, one of the Eclipse's one-word "floating
     * point functions" -- occurs in the 1984 QUEST.PR and QUEST_SERVER.PR in
     * one routine only, the FORTRAN runtime's single-precision square root
     * SQR31?3, three times, each right after an FHLV.  The NADGUG build of
     * the same routine is word for word the same except that those three
     * words are 84D8, FRDS 0,0 -- round FPAC0 to single, which is what a
     * single-precision result needs there and which the 1980 MV/8000
     * Principles of Operation does not yet list.  So the older runtime's
     * 103750 is taken to do what the newer runtime's FRDS 0,0 does. */
    case 0x87E8: FPAC[0] = fp_single(FPAC[0]); fp_setcc(FPAC[0]); break;
