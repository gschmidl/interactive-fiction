"""pascal\ADVENTURE-MJ.txt -> pascal\ADVENTURE-MJ.SYMB, the form SINTRAN
keeps a source in: CR LF line ends, even parity on every byte, ETB (027)
...
TXT = os.path.join(PORT, 'pascal', 'ADVENTURE-MJ.txt')
SYMB = os.path.join(PORT, 'pascal', 'ADVENTURE-MJ.SYMB')
...
def convert(text):
    """every line, the empty ones too (they count in the compiler's listing)"""
    out = bytearray()
    lines = text.split('\n')
