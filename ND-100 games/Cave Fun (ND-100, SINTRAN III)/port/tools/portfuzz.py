"""Play random games of Cave Fun on the port and look for trouble.

usage: python tools\portfuzz.py [SEEDS] [COMMANDS] [--keys DIR] [--first N] [--original]

Each seed plays a game with tools\play.py's random player for up to COMMANDS
commands (default 8 seeds x 200), on a scratch copy of the game file with
--raw --no-hold, under a time limit.  Printed as trouble: a run that hangs or
does not end, an unimplemented monitor call or instruction, a BASIC run-time
error, or a prompt the player does not know.  With --keys the typing of each
game is kept (DIR\portNN.keys).  --original plays build\original\ with the
game file as recovered (named CAVE-ORIG, as on the reference machine).
"""
import os
import re
import shutil
...
PROG = os.path.join(DATA, 'ADV-INTER-CB-MJ.PROG')
ORIGINAL = os.path.join(PORT, 'build', 'original', 'ADV-INTER-CB-MJ.PROG')
ORIGINAL_ADV = os.path.join(PORT, '..', 'src_original', 'CAVE-FUN-MJ.ADV')
...
def play(seed, commands, work, keep=None, first=None, original=False):
    args = [EXE, '--data', work, '--raw', '--no-hold', '--prog', ORIGINAL if original else PROG]
    player = Player(seed, commands, 'CAVE-ORIG' if original else 'CAVE-FUN')
    r = Run(args)
    typed, problem = [], None
    try:
        since = 0
...
def main():
    ...
    for seed in range(1, seeds + 1):
        work = tempfile.mkdtemp(prefix='cavefunfuzz')
        try:
            if original:
                shutil.copy(ORIGINAL_ADV, os.path.join(work, 'CAVE-ORIG.ADV'))
            else:
                shutil.copy(os.path.join(DATA, 'CAVE-FUN-MJ.ADV'), work)
            problem, found, out, done = play(seed, commands, work, keep, first, original)
        finally:
