...
/* -Z <seconds>: every ?GTOD and ?GDAY answers this instant.  QUEST seeds its
 * random numbers from the clock -- where a new player is placed, what
 * shoots at them -- so two runs of the same script only compare once the
 * clock stops moving. */
static long frozen_clock;

/* ---- the shared partition -------------------------------------------- *
 * A process's shared partition is a window of its own address space, above
 * the program, into which ?SPAGE maps blocks of a file opened with ?SOPEN.
 * The file is what is actually shared: two processes that map the same
 * blocks see each other's writes.
 *
 * Both QUEST.PR and QUEST_SERVER.PR are linked to a top of 1536 blocks
 * (95+1441 and 253+1283), so the partition starts there in both and the two
 * windows line up, which is what lets one array back them both.
 * INIT_SHARED_DATA in each program reads the partition with ?GSHPT, keeps
 * (start+size)<<10 as the base of its own area, and then grows the
 * partition by 150 blocks with ?SSHPT. */
#define SHPT_PAGE0  1536u
#define SHPT_MAXPG   256u
static unsigned shpt_start = SHPT_PAGE0, shpt_pages;
...
#define P_GSW   2
#define P_GRES  4
/* PARU.32.SR is assembled in octal: "EREOF= 30" is 030, twenty-four.  The
 * codes below were first entered as decimal, and nothing noticed until
 * QUEST_SERVER compared a read's error with 24 to tell the end of
 * USER_DATA_FILE from a real failure -- every new player then went
 * straight into ?FATAL.  Written as C octal so they read as PARU does. */
#define EREOF 030    /* end of file */
#define ERNAG 228
#define ERFDE 025    /* file does not exist */
#define ERMEM  05    /* insufficient memory */
#define ERSHP 046    /* error on shared partition set */
...
static int do_syscall32(word code)
{
    ...
    { int q = q_syscall(code, pkt); if (q != -2) return q; }
    ...
    case SC_GSHPT:                        /* AC0 = start block, AC1 = size */
        AC[0] = shpt_start; AC[1] = shpt_pages;
        if (verbose) fprintf(stderr, "   [gshpt start=%u pages=%u]\n",
                             shpt_start, shpt_pages);
        return 1;

    case SC_SSHPT: {                      /* AC0 = start block, AC1 = size */
        unsigned want = AC[1] & 0xFFFF;
        if ((AC[0] & 0xFFFF) != shpt_start || want > SHPT_MAXPG) {
            AC[0] = ERSHP; return 0;
        }
        if (want > shpt_pages)
            memset(M + MADDR(shpt_start * 1024u + shpt_pages * 1024u), 0,
                   (want - shpt_pages) * 1024u * sizeof *M);
        shpt_pages = want;
        if (verbose) fprintf(stderr, "   [sshpt start=%u pages=%u -> %05X..%05X]\n",
                             shpt_start, shpt_pages, shpt_start * 1024u,
                             (shpt_start + shpt_pages) * 1024u - 1);
        AC[0] = shpt_start;
        return 1; }
    ...
    case SC_GTOD: { time_t t = frozen_clock ? (time_t)frozen_clock : time(NULL);
                    struct tm *lt = localtime(&t);
                    AC[0] = lt->tm_sec; AC[1] = lt->tm_min; AC[2] = lt->tm_hour;
                    return 1; }
    case SC_GDAY: { time_t t = frozen_clock ? (time_t)frozen_clock : time(NULL);
                    struct tm *lt = localtime(&t);
    ...
    case SC_OPEN: case SC_SOPEN:
        ...
        /* Skip channels ?SOPEN is holding: a shared channel has no FILE*
         * of its own -- the file is held whole in memory -- so testing
        ...
        else if (name[0] == 64) { chan[ch] = stdout; chan_console[ch] = 1; }
        else {
            int wants_out = (M[MADDR(pkt + P_ISTI)] & OFOT) != 0;
            /* An open for update of a file that came with the game has to
             * work on a copy of the original, not on a new empty file:
             * QUEST_SERVER opens USER_DATA_FILE for update and reads it,
             * and an empty one sends it straight into ?FATAL.  A file that
             * is not in the data directory -- a save -- is created here as
             * before. */
...
static int do_rw32(word code, dword pkt)
{
    ...
    if (term_d200 && chan_console[ch]) return d2_io(code, pkt, fmt, rcl);

    if (code == SC_READ) {
        if (verbose) {
            int q;
            fprintf(stderr, "   [read try ch=%d fmt=%d rcl=%d open=%d pkt:",
                    ch, fmt, rcl, chan[ch] != NULL);
            for (q = 0; q < 14; q++)
                fprintf(stderr, " %04X", M[MADDR(pkt + (dword)q)]);
            fputc(']', stderr); fputc(10, stderr);
        }
        if (!chan[ch]) return 0;
        /* and the same rule the other way round: a read after a write */
        ...
        if (fmt == RF_DS) {
            ...
            /* Running off the end has to say EREOF.  QUEST_SERVER reads
             * USER_DATA_FILE record by record and stops on the end-of-file
             * error; any other code is a real failure and takes it into
             * ?FATAL, which is where a bare error return sent it. */
            if (!got) { AC[0] = EREOF; return 0; }
            n = (int)got;
    ...
    /* C stdio forbids a write straight after a read on the same stream
     * without a positioning call between them, and the Windows runtime
     * enforces it by quietly losing the write.  A relative "next record"
     * write with nothing to skip makes no fseek of its own above -- and that
     * is exactly QUEST_SERVER's save: UPDATE_USER_DATA_FILE reads its way to
     * the player's record and writes the new one in the next breath.  Every
     * character came back from a session exactly as it was created. */
...
static const char *scname(word c)
{
    ...
    case SC_GSHPT: return "?GSHPT"; case SC_SSHPT: return "?SSHPT";
    case SC_SPAGE: return "?SPAGE";
    default: return "?";
    }
}
...
static void gate_call(dword at)
{
    ...
    r = do_syscall32(code);
    /* -3 is "not yet": the task has put itself on a queue and the
     * call has to happen again when it comes off.  Winding the PC
     * back to the LCALL is the whole of the resumption mechanism --
     * nothing about a half-finished call needs remembering, because
     * none of it happened. */
    if (r == -3) { PC = at & OFFMASK; sched_yield(); return; }
    if (r < 0) {
    ...
    /* A call that ended the process -- ?RETURN, ?TERM, the last ?KILL --
     * leaves nothing to return to: schedule whoever is left instead of
     * writing a dead task's return address into the next one. */
    if (!proc[curproc].alive) { if (!halted) sched_yield(); return; }
    PC = (at + (r ? 5 : 4)) & OFFMASK;
}
