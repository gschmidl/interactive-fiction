...
static void wide_exec(word ir, dword at)
{
    ...
    /* WMSP allocates DOUBLEWORDS, not words: the wide stack's unit is the
     * 32-bit doubleword, and every caller computes its argument as
     * (bytes + 1 + 3) >> 2.  ?OPEN_SHARED_IO_FILE asks for (17+1+3)/4 = 5
     * and then copies eighteen bytes into what it got, so a word-sized WMSP
     * leaves it ten bytes short; the ?SOPEN that follows pushes its return
     * block over the middle of the name and the machine tries to open a
     * file called SHARED_DAT<the return address>LE.  ZORK.PR and FERRET.PR
     * build stack buffers with the identical idiom -- NADI 1,0 / WADI 3,0 /
     * WHLV / WHLV / WMSP -- so they were under-allocating too; it just
     * never happened to matter there. */
    ...
    case 0x8779: {                                                   /* WCMV */
        ...
        AC[2] = dp + (dword)(ds * dn);
        AC[3] = sp + (dword)(ss * n);
        /* AC1 comes back as the number of SOURCE characters not moved --
         * the Eclipse CMV's definition -- not as nought.  Compiled F77 uses
         * it.  QUEST's DISPLAY_SCREEN builds its map a row at a time with
         *
         *   AC0 = row end, AC1 = 756, AC2 = map buffer, AC3 = row
         *   WCMV / NLDAI 0,24 / WSGE 1,1 / WBR / WSTB 2,0 / WSBI 1,1
         *   NLDAI 0,756 / WSUB 1,0          -> new length = 756 - AC1
         *
         * so with AC1 zeroed every row made the map "full" at 756 characters
         * and rows two to nine were never appended: the view box stayed
         * blank while the terrain under it was perfectly good. */
    ...
    case 0xDFC8: case 0xBFC8: {                                       /* DIVS/DIVX */
                   int32_t nu;
                   /* DIVX sign-extends AC1 into AC0 first.  (ir & 0x87FF) ==
                    * 0xBFC8, the old test, can never be true; Zork's random
                    * numbers were all wrong because of it (see the Zork port's
                    * NOTES.md).  Quest executes no DIVX at all. */
                   if (ir == 0xBFC8)
    ...
    case 0xE719:                                                     /* WMESS */
        /* It is the release half of the runtime's lock.  I.LOCK takes the
         * lock with WSZBO 2,1 -- test and set bit 32 of the lock, which is
         * bit 15 of the word two on from it -- and waits with ?REC on the
         * lock's own address when the bit was already set.  I.UNLOCK then
         * does XLEF 2,2,2 to point AC2 at that same flag word and executes
         * WMESS.  So WMESS clears the flag and hands the lock to whoever is
         * waiting, by posting to the mailbox ?REC is watching.
         *
         * I.GINIT's use, three instructions in with AC0 and AC1 zero and -1
         * in AC3, has the identical operand shape and only wants the skip.
         *
         * QUEST's INIT_SCREEN is what needed this: it locks the display,
         * and with the unlock doing nothing the second lock waited for ever
         * on a message no one would ever send. */
        {
            dword fw = AC[2] & OFFMASK;
            dword box = (fw - 2) & OFFMASK;
    ...
    case 0x8498: case 0x8598: {                              /* XNDO / XWDO */
         ...
         * the stepped value is stored and loaded -- on the way out of the
         * loop too, where FORTRAN leaves the DO variable one past the limit. */
    ...
    /* LNDO / LWDO (type 17): the same loop with a long displacement -- the
     * opcode, an indirect bit and 31-bit displacement in two words, then the
     * termination offset, four words in all (MASM's table gives no length;
     * the 1984 QUEST.PR has 8698 7000024E 0038 at 17CE1A).  AC in bits 1-2,
     * index in bits 3-4, the offset counted from the displacement word. */
    case 0x8698: case 0x8798: {
        int wide = (m->op == 0x8798);
        unsigned ac = (ir >> 13) & 3;
        dword ea = xea(ir, at, 1, 0, &used);
        word off = M[MADDR(at + 3)];
        int32_t idx, lim = (int32_t)AC[ac];
        used = 4;
        idx = wide ? (int32_t)(rdw(ea) + 1) : (int32_t)(int16_t)(word)(M[MADDR(ea)] + 1);
        if (wide) wrw(ea, (dword)idx); else M[MADDR(ea)] = (word)idx;
        AC[ac] = (dword)idx;
        if (idx > lim) { PC = (dword)((int32_t)(at + 1) + (int16_t)off) & OFFMASK;
                         return; }
        break; }
